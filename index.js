...
async function startBot() {
 ...
 // Process external session string (ED BOTS compatible)
if (config.sessionID && config.sessionID.includes('!')) {
  try {
    const [header, b64data] = config.sessionID.split('!');

    if (!header || !b64data) {
      throw new Error("❌ Invalid session format. Expected 'EDBOTS! CODES'");
    }
    ...
    console.error('📡 Session : ❌ Error processing external session:', e.message);
  }
}
